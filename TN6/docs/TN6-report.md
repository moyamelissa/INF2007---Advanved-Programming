...
Le fichier `crawler_test.go` contient 27 tests unitaires et 3 bancs d'essai,
...
HTTP standard, notamment un serveur qui déclare un corps de 1 000 octets, mais
ferme la connexion après 7 octets, seul moyen de couvrir le chemin d'erreur de
`io.ReadAll` dans `fetchRobots`.
...
Atteindre 100 % de couverture a exigé de simuler des comportements impossibles avec
`httptest.NewServer`, notamment une connexion TCP fermée avant la fin du transfert,
qui a nécessité l'utilisation directe de `net.Listen` pour contrôler précisément le
comportement réseau simulé, et un octet nul dans l'URL, qui force `url.Parse` à
retourner une erreur sans aucune infrastructure réseau. La testabilité de `main()` a
été assurée via la variable de paquet `mainURLs`, substituable dans les tests sans
appel réseau réel.
...
bufferisé contrôle le degré de parallélisme et les 27 tests unitaires atteignent une

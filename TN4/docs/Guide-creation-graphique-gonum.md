...
Relever les médianes `ns/op` de `benchstat` et les diviser par 1 000 000 pour obtenir les millisecondes. Mettre à jour les variables `intMS` et `floatMS` dans `chartgen/main.go`.

## Étape 2. Structure du module chartgen

```
TN4/
└── chartgen/
    ├── go.mod    ← module chartgen, require gonum.org/v1/plot v0.14.0
...
- Code source : [`chartgen/main.go`](../chartgen/main.go)
